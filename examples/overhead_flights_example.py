...
def get_airline_logo(airline_name):
    ...
    airline_name_clean = airline_name_clean.replace("airlines", "").replace(
        "airways", ""
    )
...
def main():
    ...
    font_small.LoadFont(
        "fonts/helvR08.bdf"
    )  # Helvetica Regular 8 - readable small font
        ...
        while True:
            ...
            if flights:
                ...
                if route:
                    ...
                    dest_code = route.get("destination_iata") or route.get(
                        "destination"
                    )
                ...
                graphics.DrawText(
                    canvas, font_large, msg_x, msg_y, text_color, no_flights_msg
                )
                matrix.SwapOnVSync(canvas)
